...
public class HexMapRenderer {
	 ...
	 * cubeCoord.x = q; cubeCoord.z = r - (q - (q&1)) / 2; cubeCoord.y =
	 * -cubeCoord.x-cubeCoord.z; }
	 * 
	 * private void fromCubeCoord(GridPoint3 cubeCoord, GridPoint2 oddQ) {
	 * oddQ.x = cubeCoord.x; oddQ.y = cubeCoord.z + (cubeCoord.x -
	 * (cubeCoord.x&1)) / 2; }
	 * 
	 * private void roundToNearestHex(Vector3 cubeCoord, GridPoint3 finalCoord)
	 * {
	 * 
	 * float rx = (int)(cubeCoord.x); float ry = (int)(cubeCoord.y); float rz =
	 * (int)(cubeCoord.z);
	 * 
	 * float x_diff = Math.abs(rx - cubeCoord.x); float y_diff = Math.abs(ry -
	 * cubeCoord.y); float z_diff = Math.abs(rz - cubeCoord.z);
	 * 
	 * if(x_diff > y_diff && x_diff > z_diff) { rx = -ry-rz; } else if(y_diff >
	 * z_diff){ ry = -rx-rz; } else{ rz = -rx-ry; }
	 * 
	 * finalCoord.set((int)rx, (int)ry, (int)rz);
	 * 
	 * }
	 * 
	 * private void axialToCube( Vector2 axial, Vector3 cube) { cube.x =
	 * axial.y; cube.z = axial.x; cube.y = -cube.x-cube.z; }
	 */
